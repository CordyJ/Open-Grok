...
module GraxOutput
    ...
    proc outputGraxList (outFileNo, listNo : int)
        var s := numName (listNo)
        assert isPrefix (edgePrefix, s)
        s := s (lenEdgePrefix + 1 .. *) % Chop off prefix
        s := s (1 .. * - lenEdgeSuffix) + edgeSeparator
        put : outFileNo, '(' ..
        var firstTimeThruLoop := true
        loop
            ...
            % Possibly output a nested list here
            if isPrefix (listPrefix, nextItem) then
                stripPrefix (lenListPrefix, nextItem)
                % if strintok (nextItem) then
                    outputGraxList (outFileNo, strint (nextItem))
                % else
    ...
    % This proc will be passed as a parameter to the Grax parser
    ...
    proc putList (outFileNo : int, var s : string)
        pre isPrefix (listPrefix, s)
        stripPrefix (lenListPrefix, s)
        % if strintok (s) then
            const listNo := strint (s)
            outputGraxList (outFileNo, listNo)
        % else
    ...
    % This proc will be passed as a parameter to the Grax parser
    proc outputGraxTuple (isFact : boolean, 
		outFileNo : int, src, rel, trg : int)
        var Src := numName (src)
        var Rel := numName (rel)
        var Trg := numName (trg)
        put "###outputGraxTuple: ", Src, "=", src, " ", Rel, "=", rel, " ", Trg,
                 "=", trg
        ...
        if isPrefix (attributePrefix, Rel) then
            ...
            if isPrefix (edgePrefix, Src) then
                put : outFileNo, "(" ..
                ...
                put : outFileNo, ")" ..
            elsif isPrefix (prefixRelEntity, Src) then
                put : outFileNo, "(" ..
            ...
            if Trg = noAttribute then
                ...
                put : outFileNo, " = " ..
                if isPrefix (listPrefix, Trg) then
                    putList (outFileNo, Trg)
                % elsif isPrefix (strPrefix, Trg) then
                    % put : outFileNo, ' ' ..
            ...
            quoteIfNeeded (Src)
            quoteIfNeeded (Trg)
            put : outFileNo, "<edge type ", Rel, "\t", "alpha = ", Src, 
			"\t", "omega = ", Trg, " >"
        end if
    end outputGraxTuple
end GraxOutput
